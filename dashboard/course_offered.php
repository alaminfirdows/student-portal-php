...
<?php
  ...
  $semester_id = semester_info("SELECT MAX(id) FROM semester",'id');
  ...
  $check = $db->query("SELECT * FROM registration WHERE sem_id='$semester_id' and s_id='$s_id'");
  ...
  <table class="table table-striped">
    ...
    <?php
      while($row = $check->fetch_assoc()) {
          ...
          $query = $db->query("SELECT * FROM course_offer WHERE id='$course_offer_id'");
          ...
          <td>'.ucwords($course['semester']).'-'.$course['year'].'</td>
          ...
          <td>'.get_info("course_offer", $course['course_id'], "amount")." BDT".'</td> </tr>';
        }
...
<?php
  ...
  while (list($key, $val) = each($_POST['course_offer_id'])){
    ...
    if(empty($s_id) or empty($s_name) or empty($semester) or empty($course_offer_id)) {
      echo error("All fields are required.");
    } else if($check->num_rows>0) {
      echo error("Course Offer, on <b>$semester</b> was submitted.");
    } else {
      $insert = $db->query("INSERT into registration (s_id,cid,sem_id,amount) VALUES ('$s_id','$course_offer_id','$semester_id',".get_info("course_offer",$course_offer_id,"amount").")");
      if ($insert) {
...
*/
      ...
      $year = student_info($_SESSION['s_id'],'cur_year');
      $query = $db->query("SELECT * FROM course_offer WHERE sem_id='$semester_id' ORDER BY id");
       ...
       if($query->num_rows>0) {
        ...
        $i=0;
         ...
         if($marks < 40 && $course_code_offer = $course_registration['id']) {

        
          echo '<tr>
          <td><input type="checkbox" id="course_offer_id" name="course_offer_id[]" value="'.$row['id'].'" disabled></td>';
          ...
          <td>'.ucwords($row['semester']).'-'.$row['year'].'</td>
          ...
          <td>'.get_info("course_offer", $row['course_id'], "amount")." BDT".'</td> </tr>';
          ...
          echo '<tr>
          <td><input type="checkbox" id="course_offer_id" name="course_offer_id[]" value="'.$row['id'].'" ></td>';
...
$i++;
  ...
  <input type="hidden" name="s_name" id="s_name" value="<?php echo student_info($_SESSION['s_id'],'s_name'); ?>">
  <input type="hidden" name="semester" id="semester" value="<?php echo student_info($_SESSION['s_id'],'cur_sem'); ?>">
  <input type="hidden" name="year" id="year" value="<?php echo student_info($_SESSION['s_id'],'cur_year'); ?>">
